...
export function LoginPage() {
  ...
  return <div className="flex min-h-screen items-center justify-center bg-surface px-4 py-8"><div className="grid w-full max-w-5xl overflow-hidden rounded-lg border border-line bg-canvas shadow-xl lg:grid-cols-[0.85fr_1.15fr]">
    ...
    <div className="p-6 sm:p-10"><div className="flex items-center gap-2 lg:hidden"><SankhyaMark className="h-8 w-8" /><span className="text-sm font-semibold">SANKHYA-OPT</span></div><div className="mt-6 lg:mt-0"><p className="label-caps">Enterprise console</p><h2 className="mt-2 text-2xl font-semibold tracking-tight text-ink">Sign in locally</h2><p className="mt-1 text-sm text-ink-secondary">Use a provisioned employee identity or the attached hardware token.</p></div>
      <form onSubmit={submit} className="mt-7 space-y-4"><label className="block text-xs font-semibold text-ink-secondary">Enterprise username<input required value={username} onChange={(event) => setUsername(event.target.value)} className="mt-1.5 h-10 w-full rounded-md border border-line bg-canvas px-3 text-sm text-ink" /></label><label className="block text-xs font-semibold text-ink-secondary">Employee ID<input required value={employeeId} onChange={(event) => setEmployeeId(event.target.value)} className="mt-1.5 h-10 w-full rounded-md border border-line bg-canvas px-3 font-mono text-sm text-ink" /></label><label className="block text-xs font-semibold text-ink-secondary">Local passphrase<input required minLength={8} type="password" value={password} onChange={(event) => setPassword(event.target.value)} className="mt-1.5 h-10 w-full rounded-md border border-line bg-canvas px-3 text-sm text-ink" /></label><label className="block text-xs font-semibold text-ink-secondary">Role<select value={role} onChange={(event) => setRole(event.target.value as UserRole)} className="mt-1.5 h-10 w-full rounded-md border border-line bg-canvas px-3 text-sm text-ink">{USER_ROLES.map((item) => <option key={item}>{item}</option>)}</select></label>{error ? <p className="rounded-md border border-danger/30 bg-danger-soft px-3 py-2 text-xs text-danger">{error}</p> : null}<button type="submit" className="h-10 w-full rounded-md bg-ink text-sm font-semibold text-white hover:bg-slate-800"><LockKeyhole className="mr-2 inline h-4 w-4" />Sign in to local console</button></form>
